...
class MangaTypesetter:
    ...
    def render(self, original_image_rgb, bubbles):
        ...
        for bubble in bubbles:
            if not bubble.get("translated_text", "").strip():
                continue
            ...
            cv2.drawContours(
                image_final, contours, -1, (255, 255, 255), thickness=cv2.FILLED
            )
        ...
        for word in words:
            ...
            if w <= max_width_px:
                ...
                if current_line:
                    ...
                    for char in word:
                        if (
                            draw.textbbox((0, 0), temp_word + char, font=font)[2]
                            <= max_width_px
                        ):
                            temp_word += char
                        else:
    ...
    def _fit_text_in_mask(self, draw, text, mask):
        ...
        while font_size >= min_font_size:
            ...
            for line in lines:
                ...
                canvas_draw.text(
                    (rel_cx - line_w / 2, curr_y), line, font=font, fill=255
                )
                curr_y += line_h
